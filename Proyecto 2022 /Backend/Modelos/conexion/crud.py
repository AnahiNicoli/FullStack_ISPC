...
class Crud:
    ...
    def especial_select_case(self, argument):
        if argument == "CATEGORIAS": return "SELECT CAT.T_CAT_ID AS ID_CATEGORIA, CAT.T_CAT_NOMBRE, CAT_SUB.T_SUB_ID1 AS ID_SUBCATEGORIA, SUB.T_SUB_NOMBRE FROM CATEGORIAS AS CAT INNER JOIN CAT_SUB ON CAT.T_CAT_ID = CAT_SUB.T_CAT_ID1 AND CAT_SUB.T_C_S_ACTIVO = TRUE LEFT JOIN SUBCATEGORIAS SUB ON SUB.T_SUB_ID = CAT_SUB.T_SUB_ID1 AND SUB.T_SUB_ACTIVA = TRUE WHERE CAT.T_CAT_ACTIVA = TRUE AND CAT.T_CAT_ID = ?"
